...
> Mermaid 输入维护：源码输入使用受控 `textarea` 显式写回当前代码块，编辑期间只记录文本，收起源码后才重绘 SVG；不能把源码输入区再交给 ProseMirror 的 `contentDOM`，否则节点装饰会干扰光标。修改渲染逻辑后手动覆盖正常图表、中文节点、错误语法、切换源码、切换标签和 HTML/PDF 导出。
